...
class RDKIT_RDGENERAL_EXPORT Invariant : public std::runtime_error
{
  public:
    Invariant(const char* prefix, const char* mess, const char* expr,
        ...
        : std::runtime_error(prefix), mess_d(mess), expr_d(expr),
          prefix_d(prefix), file_dp(file), line_d(line)
    {
    }
    Invariant(const char* prefix, const std::string& mess, const char* expr,
              const char* const file, int line)
        : std::runtime_error(prefix), mess_d(mess.c_str()), expr_d(expr),
          prefix_d(prefix), file_dp(file), line_d(line)
    {
    }
...
#define POSTCONDITION(expr, mess)                                              \
    if (!(expr)) {                                                             \
        Invar::Invariant inv("Post-condition Violation", mess, #expr,          \
                             __FILE__, __LINE__);                              \
        BOOST_LOG(rdErrorLog) << "\n\n****\n" << inv << "****\n\n";            \
        throw inv;                                                             \
    }
...
#define RANGE_CHECK(lo, x, hi)                                                 \
    if ((lo) > (hi) || (x) < (lo) || (x) > (hi)) {                             \
        ...
        errstr << lo << " <= " << x << " <= " << hi;                           \
        Invar::Invariant inv("Range Error", #x, errstr.str().c_str(),          \
                             __FILE__, __LINE__);                              \
        BOOST_LOG(rdErrorLog) << "\n\n****\n" << inv << "****\n\n";            \
        throw inv;                                                             \
    }
