...
#if !defined(BOOST_NO_FUNCTION_TEMPLATE_ORDERING)
template <class T, unsigned N> std::hash_result_t hash_value(const T (&x)[N]);

template <class T, unsigned N> std::hash_result_t hash_value(T (&x)[N]);
#endif
...
template <class T> std::hash_result_t hash_value(std::complex<T> const&);

// Implementation

namespace hash_detail
{
template <class T> inline std::hash_result_t hash_value_signed(T val)
{
    ...
    const int length =
        (std::numeric_limits<T>::digits - 1) / hash_result_t_bits;
...
template <class T> inline std::hash_result_t hash_value_unsigned(T val)
{
    const int hash_result_t_bits =
        std::numeric_limits<std::hash_result_t>::digits;
    // ceiling(std::numeric_limits<T>::digits / hash_result_t_bits) - 1
...
#if BOOST_WORKAROUND(BOOST_MSVC, < 1300)
template <class T> inline void hash_combine(std::hash_result_t& seed, T& v)
#else
template <class T>
...
template <class It> inline std::hash_result_t hash_range(It first, It last)
{
    std::hash_result_t seed = 0;
...
template <class T> inline std::hash_result_t hash_range(T* first, T* last)
{
    std::hash_result_t seed = 0;
...
template <class T, unsigned N> inline std::hash_result_t hash_value(T (&x)[N])
{
    return hash_range(x, x + N);
}
#endif
...
template <class T> std::hash_result_t hash_value(std::complex<T> const& v)
{
    gboost::hash<T> hasher;
...
#define BOOST_HASH_SPECIALIZE(type)                                            \
    template <>                                                                \
    struct hash<type> : public boost::functional::detail::unary_function<      \
                            type, std::hash_result_t> {                        \
...
namespace hash_detail
{
template <bool IsPointer> struct hash_impl;

template <> struct hash_impl<true> {
    template <class T>
    struct inner
