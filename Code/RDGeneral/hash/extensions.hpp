...
{
template <bool IsArray> struct call_hash_impl {
...
template <> struct call_hash_impl<true> {
    template <class Array> struct inner {
...
template <> struct hash_impl<false> {
    template <class T>
    struct inner
...
template <bool IsConst> struct hash_impl_msvc {
    template <class T>
    struct inner
...
template <> struct hash_impl_msvc<true> {
    template <class T>
    struct inner
        : public boost::functional::detail::unary_function<T,
                                                           std::hash_result_t> {
...
template <> struct hash_impl<false> {
    template <class T> struct inner : public hash_impl_msvc2<T> {
    };
};
