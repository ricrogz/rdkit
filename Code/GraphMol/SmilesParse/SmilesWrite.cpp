...
std::string getAtomChiralityInfo(const Atom* atom)
{
    ...
    if (atString.empty() && allowNontet) {
        ...
        if (!atString.empty()) {
            // we added info about non-tetrahedral stereo, so check whether or
            // not we need to also add permutation info
            int permutation = 0;
                          ...
                          bool allHsExplicit, bool isomericSmiles)
{
    ...
    if (!allHsExplicit && inOrganicSubset(num)) {
        ...
        const INT_VECT& defaultVs =
            PeriodicTable::getTable()->getValenceList(num);
    ...
    if (!doKekule && atom->getIsAromatic() && symb[0] >= 'A' &&
        symb[0] <= 'Z') {
        symb[0] -= ('A' - 'a');
    }
    res += symb;
    ...
    case Bond::DATIVE:
        if (atomToLeftIdx >= 0 &&
            bond->getBeginAtomIdx() ==
                static_cast<unsigned int>(atomToLeftIdx)) {
            res = "->";
        } else {
            res = "<-";
        }
...
std::string
FragmentSmilesConstruct(ROMol& mol, int atomIdx,
                        ...
                        const std::vector<std::string>* bondSymbols = nullptr)
{
    ...
    if (params.doKekule) {
        if (atomsInPlay && bondsInPlay) {
            MolOps::details::KekulizeFragment(static_cast<RWMol&>(mol),
            ...
            if (ringClosureMap.count(ringIdx)) {
                ...
                while (!done) {
                    ...
                    for (mapIt = ringClosureMap.begin();
                         mapIt != ringClosureMap.end(); ++mapIt) {
                        ...
                        bool doingCXSmiles)
{
    ...
    PRECONDITION(params.rootedAtAtom < 0 ||
                     static_cast<unsigned int>(params.rootedAtAtom) <
                         mol.getNumAtoms(),
    ...
    for (unsigned fragIdx = 0; fragIdx < mols.size(); fragIdx++) {
        ...
        if (!doingCXSmiles) {
            // remove any stereo groups that may be present. Otherwise they will
            // be used in the canonicalization
      ...
      std::cout << "----------------------------" << std::endl;
        ...
        if (params.canonical) {
            if (tmol->hasProp("_canonicalRankingNumbers")) {
                ...
                bool breakTies = true;
                Canon::rankMolAtoms(*tmol, ranks, breakTies,
                                    params.doIsomericSmiles,
                                    params.doIsomericSmiles);
            }
        } else {
        ...
        subSmi = SmilesWrite::FragmentSmilesConstruct(
            *tmol, nextAtomIdx, colors, ranks, params, atomOrdering,
            bondOrdering);
    ...
    mol.setProp(common_properties::_smilesAtomOutputOrder,
                flattenedAtomOrdering, true);
    mol.setProp(common_properties::_smilesBondOutputOrder,
                flattenedBondOrdering, true);
    return result;
}
} // namespace detail
} // namespace SmilesWrite
...
MolToRandomSmilesVect(const ROMol& mol, unsigned int numSmiles,
                      unsigned int randomSeed, bool doIsomericSmiles,
                      bool doKekule, bool allBondsExplicit, bool allHsExplicit)
{
    if (randomSeed > 0) {
                                ...
                                const std::vector<std::string>* bondSymbols)
{
    ...
    if (mol.getRingInfo()->isInitialized()) {
        ...
        for (unsigned int ridx = 0; ridx < mol.getRingInfo()->numRings();
             ++ridx) {
    ...
    if (params.doIsomericSmiles) {
        if (!mol.hasProp(common_properties::_StereochemDone)) {
            ...
            for (auto aidx : atomsToUse) {
                ...
                std::string cipCode;
                if (oAt->getPropIfPresent(common_properties::_CIPCode,
                                          cipCode)) {
                    tmol.getAtomWithIdx(aidx)->setProp(
    ...
    if (params.canonical) {
        ...
        Canon::rankFragmentAtoms(
            tmol, ranks, atomsInPlay, bondsInPlay, atomSymbols, breakTies,
            params.doIsomericSmiles, params.doIsomericSmiles);
        // std::cerr << "RANKS: ";
    ...
    std::vector<Canon::AtomColors> colors(tmol.getNumAtoms(),
                                          Canon::BLACK_NODE);
    for (auto aidx : atomsToUse) {
    ...
    while (colorIt != colors.end()) {
        ...
        if (rootedAtAtom >= 0) {
            ...
            unsigned int nextRank =
                rdcast<unsigned int>(tmol.getNumAtoms()) + 1;
        ...
        auto subSmi = SmilesWrite::FragmentSmilesConstruct(
            tmol, nextAtomIdx, colors, ranks, params, atomOrdering,
            bondOrdering, &atomsInPlay, &bondsInPlay, atomSymbols, bondSymbols);
