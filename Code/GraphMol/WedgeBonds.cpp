...
std::pair<bool, INT_VECT> countChiralNbrs(const ROMol& mol, int noNbrs)
{
    ...
    for (const auto bond : mol.bonds()) {
        if (bond->getBondDir() == Bond::BEGINWEDGE ||
            ...
            if (bond->getBeginAtom()->getChiralTag() ==
                    Atom::CHI_TETRAHEDRAL_CW ||
                bond->getBeginAtom()->getChiralTag() ==
                    ...
                    int noNbrs)
{
    ...
    for (const auto bond : mol.atomBonds(atom)) {
        ...
        if (resSoFar.find(bid) == resSoFar.end()) {
            ...
            // prefer neighbors that are nonchiral or have as few chiral
                             ...
                             const BondWedgingParameters* params)
{
  ...
  std::cerr << std::endl;
    ...
    for (auto idx : indices) {
        ...
        auto bnd1 =
            detail::pickBondToWedge(atom, mol, nChiralNbrs, res, noNbrs);
        if (bnd1 >= 0) {
            res[bnd1] = idx;
        }
    }
...
void addSecondWedgeAroundAtom(ROMol& mol, Bond* refBond, const Conformer* conf)
{
    ...
    if (bondToWedge && minAngle < 2 * M_PI / 3) {
        bondToWedge->setBondDir(refBond->getBondDir() ==
...
void wedgeBond(Bond* bond, unsigned int fromAtomIdx, const Conformer* conf)
{
    ...
    Bond::BondDir dir =
        detail::determineBondWedgeState(bond, fromAtomIdx, conf);
    if (dir == Bond::BEGINWEDGE || dir == Bond::BEGINDASH) {
        bond->setBondDir(dir);
    }
}
