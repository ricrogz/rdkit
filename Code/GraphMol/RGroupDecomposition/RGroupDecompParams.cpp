                       ...
                       unsigned int autoLabels)
{
    ...
    while (nbrIdx != endNbrs) {
        ...
        if (neighborAtom->getAtomicNum() == 0 &&
            neighborAtom->getDegree() == 1 &&
                                                ...
                                                const RWMol* alignCore)
{
    ...
    if (labels == AutoDetect) {
        ...
        if (!autoLabels) {
            BOOST_LOG(rdWarningLog)
                << "RGroupDecomposition auto detect found no "
    ...
    int maxLabel = 1;
    // makes no sense to do MCS alignment if we are only matching at user
    // defined R-Groups
    ...
    for (auto atom : core.atoms()) {
        ...
        if (atom->hasProp(RLABEL)) {
            // set from MCS match
            if (setLabel(atom, atom->getProp<int>(RLABEL), foundLabels,
                         maxLabel, relabel, Labelling::INTERNAL_LABELS)) {
                found = true;
            }
        }
        ...
        if (atom->getAtomicNum() == 0) {
            if (!found && (autoLabels & MDLRGroupLabels)) {
                ...
                if (atom->getPropIfPresent<unsigned int>(
                    ...
                    if (setLabel(atom, rdcast<int>(rgroup), foundLabels,
                                 maxLabel, relabel, Labelling::RGROUP_LABELS)) {
                        found = true;
            ...
            if (!found && (autoLabels & IsotopeLabels) &&
                atom->getIsotope() > 0) {
            ...
            if (!found && (autoLabels & AtomMapLabels) &&
                atom->getAtomMapNum() > 0) {
                if (setLabel(atom, rdcast<int>(atom->getAtomMapNum()),
                             foundLabels, maxLabel, relabel,
                             Labelling::ATOMMAP_LABELS)) {
                    checkNonTerminal(*atom);
                    found = true;
                }
            }
            ...
            if (!found && (autoLabels & DummyAtomLabels) &&
                ...
                if (setLabel(atom, defaultDummyStartLabel, foundLabels,
                             maxLabel, forceRelabellingWithDummies,
                             Labelling::DUMMY_LABELS)) {
                    found = true;
                }
            }
        }
        ...
        if (!found && (autoLabels & AtomIndexLabels)) {
            ...
            if (setLabel(atom, indexOffset - atom->getIdx(), foundLabels,
                         maxLabel, relabel, Labelling::INDEX_LABELS)) {
            }
            found = true;
        }
...
void RGroupDecompositionParameters::checkNonTerminal(const Atom& atom) const
{
    ...
    BOOST_LOG(rdWarningLog) << "Non terminal R group defined.  To allow set "
