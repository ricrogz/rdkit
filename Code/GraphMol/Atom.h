...
class RDKIT_GRAPHMOL_EXPORT Atom : public RDProps {
  ...
  */
  ...
  //! Get the type of the query, or an empty string if we don't have one
  ...
  */
  ...
  //! returns true if our query details match those of Atom \c what, which must
  //! have a query
  bool QueryMatch(Atom const *what) const;
