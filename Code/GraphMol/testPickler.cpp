...
void testIssue164()
{
    ...
    std::string smi =
        ...
        "="
        "O)C(NC(C(NC(CO)C(NC(C)C(NC(CCC(O)=O)C(NC(C)C(NC(Cc3ccccc3)C(=O)N5C(C("
        "NC("
        "CC(C)C)C(NC(C(NC(C(O)=O)Cc4ccccc4)=O)CCC(O)=O)=O)=O)CCC5)=O)=O)=O)=O)="
        "O)"
...
void testRadicals()
{
    BOOST_LOG(rdErrorLog) << "-------------------------------------"
                          << std::endl;
    BOOST_LOG(rdErrorLog) << "Testing radical handling in pickles."
                          << std::endl;
...
void testIssue3202580()
{
...

    {
        ...
        std::cerr << "  mass! " << m1->getAtomWithIdx(0)->getMass()
                  << std::endl;
...
void testIssue3316407()
{
...

    {
        ...
        TEST_ASSERT(
            m2->getAtomWithIdx(3)->hasProp(common_properties::dummyLabel));
        TEST_ASSERT(
            m2->getAtomWithIdx(4)->hasProp(common_properties::dummyLabel));
...
void testIssue280()
{
...

    {
        ...
        RWMol* m2 = new RWMol(pickle);
        TEST_ASSERT(m2);
        TEST_ASSERT(m2->getAtomWithIdx(0)->hasProp(
            common_properties::molAtomMapNumber));
        TEST_ASSERT(m2->getAtomWithIdx(1)->hasProp(
            common_properties::molAtomMapNumber));
        int iv;
...
void testIssue285()
{
...

    {
        ...
        TEST_ASSERT(m2);
        TEST_ASSERT(
            m2->getAtomWithIdx(0)->hasProp(common_properties::dummyLabel));
...
void testGithub1999()
{
    BOOST_LOG(rdInfoLog) << "-----------------------\n";
    BOOST_LOG(rdInfoLog)
        << "Testing Github 1999: pickling atom map numbers >127 "
...
void testEnhancedStereoChemistry()
{
    ...
    // add 3 stereo groups
    {
        ...
        std::vector<Atom*> atoms0 = {
            {m.getAtomWithIdx(0), m.getAtomWithIdx(1)}};
        groups.emplace_back(RDKit::StereoGroupType::STEREO_ABSOLUTE,
                            std::move(atoms0));
        std::vector<Atom*> atoms1 = {
            {m.getAtomWithIdx(2), m.getAtomWithIdx(3)}};
        groups.emplace_back(RDKit::StereoGroupType::STEREO_OR,
                            std::move(atoms1));
        std::vector<Atom*> atoms2 = {
            {m.getAtomWithIdx(4), m.getAtomWithIdx(5)}};
        groups.emplace_back(RDKit::StereoGroupType::STEREO_AND,
                            std::move(atoms2));
    ...
    for (unsigned i = 0u; i < 3; ++i) {
        TEST_ASSERT(ref_groups[i].getGroupType() ==
                    new_groups[i].getGroupType());
    }
}
...
void testCustomPickler()
{
    ...
    TEST_ASSERT(m.getAtomWithIdx(0)->getProp<ExplicitBitVect>("bv").getBit(
                    100) == true);
    TEST_ASSERT(m.getAtomWithIdx(0)->getProp<ExplicitBitVect>("bv").getBit(
                    101) == false);
...
void testHistoricalConfs()
{
...

    {
        std::string pklName = getenv("RDBASE");
        pklName +=
            "/Code/GraphMol/test_data/mol_with_confs.pkl"; // written with
                                                           // v2021.03.1
        std::ifstream inStream(pklName.c_str(), std::ios_base::binary);
        ROMol m1;
...

    {
        std::string pklName = getenv("RDBASE");
        pklName +=
            "/Code/GraphMol/test_data/mol_with_double_confs.pkl"; // written
                                                                  // with
                                                                  // v2021.03.1
        std::ifstream inStream(pklName.c_str(), std::ios_base::binary);
        ROMol m1;
...
void testAdditionalQueryPickling()
{
    ...
    m1->getAtomWithIdx(3)->expandQuery(
        sq, Queries::CompositeQueryType::COMPOSITE_OR);
