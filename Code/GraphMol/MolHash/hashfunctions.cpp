                     ...
                     unsigned additionalSkips = 0)
{
    unsigned int cxflagsToSkip =
        additionalSkips | RDKit::SmilesWrite::CX_COORDS;
...
RDKit::Bond* NMRDKitMolNewBond(RDKit::RWMol* mol, RDKit::Atom* src,
                               RDKit::Atom* dst, unsigned int order, bool arom)
{
                               ...
                               unsigned int part)
{
    ...
    for (unsigned int i = 0; i < 119; i++) {
        ...
        if (hist[elem] > 0) {
            result += symbol[elem];
            if (hist[elem] > 1) {
                snprintf(buffer, 16, "%u", hist[elem]);
                result += buffer;
            }
        }
    }
...
{
std::string
convertToSmilesWithCXFlags(const RWMol& mol, bool doingCXSmiles,
                           ...
                           unsigned cxFlagsToSkip = 0)
{
...
#endif
    for (auto bptr : mol->bonds()) {
        // If this has already been considered or is not a possible starting
        // bond, then skip it
        ...
        for (const auto atm : std::vector<const Atom*>{bptr->getBeginAtom(),
                                                       bptr->getEndAtom()}) {
            ...
            for (const auto nbrBond : mol->atomBonds(atm)) {
                ...
                // if the bond is unsaturated or to an atom with free Hs,
                // include it: if (isUnsaturatedBond(nbrBond) ||
                // oatom->getTotalNumHs()) {

#ifdef VERBOSE_HASH
                std::cerr << "  check neighbor1 " << nbrBond->getIdx()
                          << " from " << atm->getIdx() << "-" << oatom->getIdx()
                          << std::endl;
                std::cerr << "    " << bondsConsidered[nbrBond->getIdx()]
                          << " icao " << isCandidateAtom(oatom) << " hsbo "
        ...
        while (!bq.empty()) {
            ...
            for (const auto atm : std::vector<const Atom*>{bnd->getBeginAtom(),
                                                           bnd->getEndAtom()}) {
...
#ifdef VERBOSE_HASH
                    std::cerr << "  check neighbor " << nbrBnd->getIdx()
                              << " from " << atm->getIdx() << "-"
                              << oatom->getIdx() << std::endl;
                    std::cerr << "    " << bondsConsidered[nbrBnd->getIdx()]
                              << " icao " << isCandidateAtom(oatom) << " hsbo "
                              << hasStartBond(oatom, startBonds) << " unsat "
...
#ifdef VERBOSE_HASH
            std::cerr << "REJECT CONJ: " << conjSystem << " hetero "
                      << activeHeteroHs << " donor " << possibleDonorCs
                      << std::endl;
#endif
        }
    }
    ...
    char buffer[32];
    if (!proto) {
        snprintf(buffer, 32, "_%d_%d", hcount, charge);
    } else {
        snprintf(buffer, 32, "_%d", hcount - charge);
    }
    result += buffer;
    if (useCXSmiles) {
        addCXExtensions(mol, result, cxFlagsToSkip | SmilesWrite::CX_RADICALS);
                         ...
                         unsigned cxFlagsToSkip = 0)
{
    ...
    for (auto bptr : mol->bonds()) {
        if (bptr->getBondType() != Bond::SINGLE &&
            (bptr->getIsConjugated() ||
             bptr->getBeginAtom()->getAtomicNum() != 6 ||
            ...
            if (deg < 2) {
                if (deg == 1) { // i.e. not 0 and the last atom in the molecule
                    for (const auto& nbri : boost::make_iterator_range(
                        ...
                        nbr->setNumExplicitHs(hcount +
                                              NMRDKitBondGetOrder(bptr));
...
std::string SmallWorldHash(RWMol* mol, bool brl)
{
    ...
    if (brl) {
        ...
        snprintf(buffer, 64, "B%uR%uL%u", bcount, rcount, lcount);
    } else {
        snprintf(buffer, 64, "B%uR%u", bcount, rcount);
    }
    return buffer;
}
...
std::string ArthorSubOrderHash(RWMol* mol)
{
    ...
    for (auto aptr : mol->atoms()) {
        ...
        case 53: // Iodine
            ...
            if (charge == 0) {
                unsigned int valence = aptr->getTotalValence();
                if (valence != 1 && valence != 3 && valence != 5 &&
                    valence != 7) {
                    rcount++;
                }
            }
    ...
    snprintf(buffer, 256, "%04x%04x%02x%04x%04x%06x%02x%02x%02x", acount,
             bcount, pcount, ccount, ocount, zcount, rcount, qcount, icount);
    return buffer;
}
} // namespace
                    ...
                    unsigned cxFlagsToSkip)
{
    ...
    case HashFunction::AtomBondCounts:
        snprintf(buffer, 32, "%u,%u", mol->getNumAtoms(), mol->getNumBonds());
        result = buffer;
        break;
    ...
    case HashFunction::DegreeVector: {
        unsigned int dv[4];
        DegreeVector(mol, dv);
        snprintf(buffer, 32, "%u,%u,%u,%u", dv[0], dv[1], dv[2], dv[3]);
        result = buffer;
    } break;
