...
template <class Graph> struct Pair {
    node_id n1, n2;
    bool hasiter{false};
 ...
 */
static inline bool nodeInfoComp1(const NodeInfo& a, const NodeInfo& b)
{
    if (a.out < b.out) {
        return true;
    }
 ...
 */
static inline int nodeInfoComp2(const NodeInfo& a, const NodeInfo& b)
{
    if (!a.in && b.in) {
        return 1;
    }
 ...
 * start.
 *--------------------------------------------------*/
template <class Graph> node_id* SortNodesByFrequency(const Graph* g)
{
...
class VF2SubState
{
  ...
  public:
    VF2SubState(Graph* ag1, Graph* ag2, VertexCompatible& avc,
        ...
        : g1(ag1), g2(ag2), vc(avc), ec(aec), mc(amc), n1(num_vertices(*ag1)),
          n2(num_vertices(*ag2))
    {
        if (sortNodes) {
    ...
    std::cerr<<std::endl;
        ...
        if (t1_len > core_len && t2_len > core_len) {
             ...
             * core_1). Rather than select pair.n2 from all atoms (0...n2) we
             * can select it from the neighbors of this mapped atom
             * (0...deg(nbor)) since it must also be adajcent to this mapped
             * atom!
             */
            if (!pair.hasiter) {
            ...
            // Optimisation: if the order vector is laid out in a DFS/BFS then
             ...
             DoubleBackInsertionSequence& F, unsigned int max_results = 1000)
{
    detail::VF2SubState<const Graph, VertexLabeling, EdgeLabeling,
                        MatchChecking>
        s0(&g1, &g2, vertex_labeling, edge_labeling, match_checking, false);
    std::unique_ptr<detail::node_id[]> ni1(
        new detail::node_id[num_vertices(g1)]);
    std::unique_ptr<detail::node_id[]> ni2(
        new detail::node_id[num_vertices(g2)]);
