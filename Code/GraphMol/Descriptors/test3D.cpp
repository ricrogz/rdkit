...
void testPMIEdges()
{
...

    {
        ...
        TEST_ASSERT(val >= 10);
        TEST_ASSERT(
            RDKit::Descriptors::PMI3(*m) - RDKit::Descriptors::PMI2(*m) < 1e-2);
    ...
    }
    {
        ...
        TEST_ASSERT(
            RDKit::Descriptors::PMI2(*m) - RDKit::Descriptors::PMI1(*m) < 1e-2);
        TEST_ASSERT(
            RDKit::Descriptors::PMI3(*m) - RDKit::Descriptors::PMI1(*m) > 10);
        delete m;
    }
    {
        std::string pathName = getenv("RDBASE");
        ...
        TEST_ASSERT(
            RDKit::Descriptors::PMI2(*m) - RDKit::Descriptors::PMI1(*m) < 1e-2);
        TEST_ASSERT(
            RDKit::Descriptors::PMI3(*m) - RDKit::Descriptors::PMI1(*m) > 1);
        delete m;
    }
...
void testPMI2()
{
    ...
    while (!reader.atEnd()) {
        ...
        TEST_ASSERT(
            compare("pmi1", m->getProp<double>("pmi1"), pmi1, pmi1 / 100));
        TEST_ASSERT(
            compare("pmi2", m->getProp<double>("pmi2"), pmi2, pmi2 / 100));
        TEST_ASSERT(
            compare("pmi3", m->getProp<double>("pmi3"), pmi3, pmi3 / 100));

        TEST_ASSERT(
            compare("npr1", m->getProp<double>("npr1"), npr1, npr1 / 100));
        TEST_ASSERT(
            compare("npr2", m->getProp<double>("npr2"), npr2, npr2 / 100));
        delete m;
    }
    BOOST_LOG(rdErrorLog) << "  done" << std::endl;
}
...
void testInertialShapeFactor()
{
    ...
    BOOST_LOG(rdErrorLog) << "    Inertial shape factor." << std::endl;
    {
        ...
        std::unique_ptr<RDKit::ROMol> m(
            MolFileToMol(sdfName, sanitize, removeHs));
        TEST_ASSERT(m);
