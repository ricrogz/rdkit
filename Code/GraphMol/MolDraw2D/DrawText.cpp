...
// ****************************************************************************
double DrawText::baseFontSize() const
{
    return base_font_size_;
}
// ****************************************************************************
void DrawText::setBaseFontSize(double val)
{
    base_font_size_ = val;
}
    ...
    if (talign == TextAlignType::START || talign == TextAlignType::END) {
        ...
        for (size_t i = 0; i < rects.size(); ++i) {
            if (draw_modes[i] == TextDrawType::TextDrawNormal) {
                ...
                // sometimes the rect is in a coordinate frame where +ve y is
                // down, sometimes it's up.  For these purposes, we don't care
                // so long as the y_max is larger than the y_min.  We probably
                // don't need to do all the tests for x_min and x_max;
                x_min = std::min(bl.x, x_min);
                x_min = std::min(tr.x, x_min);
                y_min = std::min(bl.y, y_min);
                y_min = std::min(tr.y, y_min);
                         ...
                         const std::vector<char>& draw_chars)
{
    ...
    for (size_t i = 0; i < rects.size(); ++i) {
        ...
        setFontScale(
            full_scale * selectScaleFactor(draw_chars[i], draw_modes[i]), true);
