                                   ...
                                   const MatchVectType& aidToFid)
{
    ...
    setBitId(-1);
    INT_MAP_INT
    aIdxMap; // a map from atom id in omol to the new atoms id in mol
...
bool FragCatalogEntry::match(const FragCatalogEntry* other, double tol) const
{
    ...
    // REVIEW: need an overload of feq that handles tuples in MolOps, or
    // wherever DiscrimTuple is defined
...
Subgraphs::DiscrimTuple FragCatalogEntry::getDiscrims() const
{
    ...
    if (this->hasProp(common_properties::Discrims)) {
        ...
        // create invariant additions to reflect the functional groups attached
        // to the atoms
        ...
        res = Subgraphs::calcPathDiscriminators(*dp_mol, path, true,
                                                &funcGpInvars);
...
void FragCatalogEntry::toStream(std::ostream& ss) const
{
    ...
    for (const auto& iivmci : d_aToFmap) {
        ...
        tmpInt = tmpVect.size();
        streamWrite(ss, tmpInt);
        for (INT_VECT_CI ivci = tmpVect.begin(); ivci != tmpVect.end();
             ivci++) {
            tmpInt = *ivci;
            streamWrite(ss, tmpInt);
        }
