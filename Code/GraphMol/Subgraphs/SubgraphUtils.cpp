                    ...
                    INT_MAP_INT& atomIdxMap)
{
    ...
    if (useQuery) {
        // have to do this in two different blocks because of issues with
        // variable scopes.
                                    ...
                                    std::vector<std::uint32_t>* extraInvars)
{
    ...
    for (unsigned int cycle = 0; cycle < nCycles; ++cycle) {
        ...
        for (int pathIter : path) {
            ...
            if (useBO) {
                gboost::hash_combine(
                    v1, static_cast<uint32_t>(bond->getBondType()));
                gboost::hash_combine(
                    v2, static_cast<uint32_t>(bond->getBondType()));
            }
...
//  atoms)
//
PATH_LIST uniquifyPaths(const ROMol& mol, const PATH_LIST& allPaths, bool useBO)
{
    PATH_LIST res;
