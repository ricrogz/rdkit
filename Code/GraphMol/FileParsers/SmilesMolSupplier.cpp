...
ROMol* SmilesMolSupplier::processLine(std::string inLine)
{
    ...
    try {
        ...
        for (unsigned int col = 0; col < recs.size(); col++) {
            if (static_cast<int>(col) == d_smi ||
                static_cast<int>(col) == d_name) {
                continue;
            }
        ...
        BOOST_LOG(rdErrorLog)
            << "ERROR: Smiles parse error on line " << d_line << "\n";
    ...
    while (d_molpos.size() <= idx) {
        ...
        if (nextP < 0) {
            ...
            if (d_molpos.size() == idx + 1 && df_end) {
                // boundary condition: we could read the point we were looking
                // for but not the next one. indicate that we've reached EOF:
                dp_inStream->clear();
