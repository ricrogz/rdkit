    ...
    size_t sizeOutputQueue)
{
    ...
    initFromSettings(true, delimiter, smilesColumn, nameColumn, titleLine,
                     sanitize, numWriterThreads, sizeInputQueue,
                     sizeOutputQueue);
    startThreads();
    ...
    size_t sizeOutputQueue)
{
    ...
    d_inputQueue = new ConcurrentQueue<
        std::tuple<std::string, unsigned int, unsigned int>>(d_sizeInputQueue);
    d_outputQueue =
...
ROMol*
MultithreadedSmilesMolSupplier::processMoleculeRecord(const std::string& record,
                                                      unsigned int lineNum)
{
    ROMol* res = nullptr;
