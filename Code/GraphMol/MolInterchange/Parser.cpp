              ...
              const JSONParseParameters& params)
{
    ...
    if (params.useHCounts) {
        ...
        at->setNumExplicitHs(
            getIntDefaultValue("impHs", atomVal, atomDefaults));
    }
...
template <typename T> void parseProperties(T& obj, const rj::Value& propsVal)
{
    ...
    for (const auto& sgVal : sgVals.GetArray()) {
        if (!sgVal.HasMember("type")) {
            throw FileParseException(
                "Bad Format: stereogroup does not have a type");
        }
        if (!sgVal.HasMember("atoms")) {
            throw FileParseException(
                "Bad Format: stereogroup does not have atoms");
        }
...
void readSubstanceGroups(RWMol* mol, const rj::Value& sgVals)
{
    ...
    for (const auto& sgVal : sgVals.GetArray()) {
        ...
        if (sg.getPropIfPresent("SUBTYPE", pval) &&
            ...
            throw FileParseException(
                (boost::format("Bad Format: substance group SUBTYPE '%s' not "
                               "recognized") %
                 pval)
                    .str());
        }
        if (sg.getPropIfPresent("CONNECT", pval) &&
            ...
            throw FileParseException(
                (boost::format("Bad Format: substance group CONNECT type '%s' "
                               "not recognized") %
                 pval)
                    .str());
        }
        ...
        if (sgVal.HasMember("brackets")) {
            ...
            for (const auto& brk : brks) {
                ...
                for (const auto& pt : brk.GetArray()) {
                    ...
                    if (pta.Size() != 3) {
                        throw FileParseException(
                            "Bad Format: bracket point doesn't have three "
                            "coordinates");
                    }
        ...
        if (sgVal.HasMember("cstates")) {
            ...
            for (const auto& cstat : cstats) {
                ...
                if (cstat.HasMember("vector")) {
                    ...
                    if (pta.Size() != 3) {
                        throw FileParseException(
                            "Bad Format: cstate vector doesn't have three "
                            "coordinates");
                    }
                    RDGeom::Point3D loc(pta[0].GetDouble(), pta[1].GetDouble(),
                                        pta[2].GetDouble());
...
void readConformer(Conformer* conf, const rj::Value& confVal)
{
    ...
    for (const auto& ptVal : confVal["coords"].GetArray()) {
        ...
        if (arr.Size() != dim) {
            throw FileParseException(
                "coordinate contains wrong number of values");
        }
    ...
    }
    {
        ...
        if (miter != repVal.MemberEnd()) {
            ...
            for (unsigned int idx = 0; idx != mol->getNumAtoms(); ++idx) {
                ...
                if (!val.IsDouble()) {
                    throw FileParseException(
                        "Bad Format: partial charge not double");
                }
                mol->getAtomWithIdx(idx)->setProp(
                    common_properties::_GasteigerCharge, val.GetDouble());
            }
        }
    }
}
    ...
    }
    {
        ...
        if (miter != repVal.MemberEnd()) {
            ...
            for (const auto& val : miter->value.GetArray()) {
                if (!val.IsObject()) {
                    throw FileParseException(
                        "Bad Format: atomQuery not object");
                }
    ...
    }
    {
        ...
        if (miter != repVal.MemberEnd()) {
            ...
            for (const auto& val : miter->value.GetArray()) {
                if (!val.IsObject()) {
                    throw FileParseException(
                        "Bad Format: bondQuery not object");
                }
                if (!val.HasMember("tag")) {
                    // nothing here, continue
                    continue;
...
{
    PRECONDITION(mol, "no molecule");
    PRECONDITION(repVal["name"].GetString() ==
    ...
    }
    {
        ...
        if (miter != repVal.MemberEnd()) {
            for (const auto& val : miter->value.GetArray()) {
                if (!val.IsInt()) {
                    throw FileParseException(
                        "Bad Format: aromaticAtom not int");
                }
    ...
    }
    {
        ...
        if (miter != repVal.MemberEnd()) {
            for (const auto& val : miter->value.GetArray()) {
                if (!val.IsInt()) {
                    throw FileParseException(
                        "Bad Format: aromaticBond not int");
                }
                mol->getBondWithIdx(val.GetInt())->setIsAromatic(true);
                if (params.setAromaticBonds) {
                    mol->getBondWithIdx(val.GetInt())
                        ->setBondType(Bond::AROMATIC);
                }
            }
        }
    }
                ...
                const JSONParseParameters& params)
{
    ...
    if (molval.HasMember("extensions")) {
        for (const auto& propVal : molval["extensions"].GetArray()) {
            ...
            if (propVal["name"].GetString() ==
                std::string("rdkitRepresentation")) {
                readRDKitRepresentation(mol, propVal, params);
            } else if (propVal["name"].GetString() ==
...
DocToMols(rj::Document& doc, const JSONParseParameters& params)
{
    ...
    if (rj::GetValueByPointer(doc, "/defaults/atom")) {
        ...
        if (!atomDefaults_.IsObject()) {
            throw FileParseException(
                "Bad Format: atomDefaults is not an object");
        }
    }
    ...
    if (rj::GetValueByPointer(doc, "/defaults/bond")) {
        ...
        if (!bondDefaults_.IsObject()) {
            throw FileParseException(
                "Bad Format: bondDefaults is not an object");
        }
    }
...
std::vector<boost::shared_ptr<ROMol>>
JSONDataToMols(const std::string& jsonBlock, const JSONParseParameters& params)
{
    rj::Document doc;
