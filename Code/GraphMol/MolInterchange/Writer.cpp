...
{
    rj::Value descr;
    descr.SetString(query.getDescription().c_str(),
                    query.getDescription().size(), doc.GetAllocator());
    ...
    if (typeid(query) == typeid(RecursiveStructureQuery)) {
        ...
        rjQuery.AddMember("tag", MolPickler::QUERY_RECURSIVE,
                          doc.GetAllocator());
        ...
        case 1: {
            auto v =
                boost::get<std::tuple<MolPickler::Tags, int32_t>>(qdetails);
        ...
        case 4: {
            auto v =
                boost::get<std::tuple<MolPickler::Tags, std::set<int32_t>>>(
                    qdetails);
            rjQuery.AddMember("tag", std::get<0>(v), doc.GetAllocator());
             ...
             const rj::Value& rjDefaults, const JSONWriteParameters& params)
{
    ...
    if (bo < 0) {
        bo = 0;
        BOOST_LOG(rdWarningLog)
            << " unrecognized bond type " << bond.getBondType()
    ...
    if (auto sbIter = inv_stereoBondlookup.find(bond.getStereo());
        ...
        BOOST_LOG(rdWarningLog)
            << " unrecognized bond stereo " << bond.getStereo()
...
void addStereoGroup(const StereoGroup& sg, rj::Value& rjSG, rj::Document& doc)
{
    ...
    addStringVal(rjSG, "type", inv_stereoGrouplookup.at(sg.getGroupType()),
                 doc);
            ...
            const JSONWriteParameters& params)
{
    ...
    if (params.useRDKitExtensions && !getSubstanceGroups(mol).empty()) {
        ...
        rjMol.AddMember("substanceGroups", rjSubstanceGroups,
                        doc.GetAllocator());
    }
    ...
    if (hasArom) {
        {
            ...
            representation.AddMember("aromaticAtoms", rjArr,
                                     doc.GetAllocator());
        }
        {
            rj::Value rjArr(rj::kArrayType);
            ...
            representation.AddMember("aromaticBonds", rjArr,
                                     doc.GetAllocator());
        }
    }
    {
    ...
    }
    {
        ...
        for (const auto& atom : mol.atoms()) {
            ...
            if (atom->getPropIfPresent(common_properties::_CIPCode, cip)) {
                ...
                rjElement.PushBack(rj::Value(atom->getIdx()),
                                   doc.GetAllocator());
    ...
    if (mol.getAtomWithIdx(0)->hasProp(common_properties::_GasteigerCharge)) {
        ...
        for (const auto& at : mol.atoms()) {
            ...
            if (at->hasProp(common_properties::_GasteigerCharge)) {
                rjval =
                    at->getProp<double>(common_properties::_GasteigerCharge);
            } else {
                rjval = 0.0;
            }
...
MolsToJSONData<ROMol*>(const std::vector<ROMol*>&, const JSONWriteParameters&);
template RDKIT_MOLINTERCHANGE_EXPORT std::string
MolsToJSONData<RWMol*>(const std::vector<RWMol*>&, const JSONWriteParameters&);
template RDKIT_MOLINTERCHANGE_EXPORT std::string
MolsToJSONData<const ROMol*>(const std::vector<const ROMol*>&,
                             const JSONWriteParameters&);
template RDKIT_MOLINTERCHANGE_EXPORT std::string
MolsToJSONData<const RWMol*>(const std::vector<const RWMol*>&,
                             const JSONWriteParameters&);
