                            ...
                            MolStack& molStack)
{
    ...
    PRECONDITION(dblBond->getStereoAtoms().size() >= 2,
                 "bad bond stereo atoms");
    ...
    auto findNeighborBonds =
        [&mol, &dblBond, &bondDirCounts, &bondVisitOrders,
         &firstVisitOrder](auto atom, auto& firstNeighborBond,
            ...
            for (const auto bond : mol.atomBonds(atom)) {
                ...
                if (!firstNeighborBond ||
                    bondVisitOrders[bondIdx] < firstVisitOrder) {
                    if (firstNeighborBond) {
    ...
    if (!dir1Set && !dir2Set) {
        ...
        // at least one of the bonds on atom1 has its directionality set
        // already:
        if (bondDirCounts[firstFromAtom1->getIdx()] > 0) {
            ...
            if (secondFromAtom1) {
                ...
                // they are compatible:
                if (firstFromAtom1->getBondDir() ==
                        secondFromAtom1->getBondDir() &&
                    bondDirCounts[firstFromAtom2->getIdx()]) {
                    CHECK_INVARIANT(((firstFromAtom1->getBeginAtomIdx() ==
                                      atom1->getIdx()) ^
                                     (secondFromAtom1->getBeginAtomIdx() ==
                                      atom1->getIdx())),
                                    "inconsistent state");
                }
            }
        ...
        setFromBond1 = false;
        // at least one of the bonds on atom2 has its directionality set
        // already:
        if (bondDirCounts[firstFromAtom2->getIdx()] > 0) {
            ...
            if (secondFromAtom2) {
                // both bonds have their directionalities set, make sure
                // they are compatible:
                if (firstFromAtom2->getBondDir() ==
                        secondFromAtom2->getBondDir() &&
                    bondDirCounts[firstFromAtom1->getIdx()]) {
                    CHECK_INVARIANT(((firstFromAtom2->getBeginAtomIdx() ==
                                      atom2->getIdx()) ^
                                     (secondFromAtom2->getBeginAtomIdx() ==
                                      atom2->getIdx())),
                                    "inconsistent state");
                }
            }
    ...
    if (setFromBond1) {
        ...
        if (atom1->getDegree() ==
                3 && // atom1ControllingBond == firstFromAtom1 &&
    ...
    if (atom1->getDegree() == 3 && secondFromAtom1) {
        if (!bondDirCounts[secondFromAtom1->getIdx()]) {
            // This bond (the second bond from the starting atom of the double
            // bond) is a special case.  It's going to appear in a branch in the
            // smiles:
            //     X\C(\Y)=C/Z
            //         ^
            //         |- here
            // so it actually needs to go down with the *same* direction as the
            // bond that's already been set (because "pulling the bond out of
            // the branch" reverses its direction). A quick example.  This
            // SMILES:
            //     F/C(\Cl)=C/F
            ...
            // UNLESS the bond is not in a branch (in the smiles) (e.g.
            // firstFromAtom1 branches off a cycle, and secondFromAtom1 shows up
            // at the end of the cycle). This was Github Issue #2023, see it for
            // an example.
    ...
    if (atom2->getDegree() == 3 && secondFromAtom2) {
        if (!bondDirCounts[secondFromAtom2->getIdx()]) {
            // Here we set the bond direction to be opposite the other one
            // (since both come after the atom connected to the double bond).
    ...
    if (setFromBond1) {
        ...
        if (bondVisitOrders[atom1ControllingBond->getIdx()] >
            ...
            if (bondDirCounts[atom1ControllingBond->getIdx()] == 1) {
                ...
                // the controlling bond at atom1 is being set by someone else,
                // flip the direction on the atom2 bond instead: std::cerr<<"
                // switcheroo 2"<<std::endl;
        ...
        if (inBondIdx < 0 ||
            ...
            if (!doRandom) {
                if (colors[otherIdx] == GREY_NODE) {
                    rank -= static_cast<int>(MAX_BONDTYPE + 1) * MAX_NATOMS *
                            MAX_NATOMS;
                    if (!bondSymbols) {
                        rank += static_cast<int>(MAX_BONDTYPE -
                                                 theBond->getBondType()) *
                                MAX_NATOMS;
                    } else {
                        const std::string& symb =
                            (*bondSymbols)[theBond->getIdx()];
                        std::uint32_t hsh =
                            gboost::hash_range(symb.begin(), symb.end());
    ...
    for (auto& possible : possibles) {
        ...
        case WHITE_NODE:
            ...
            dfsFindCycles(mol, possibleIdx, bond->getIdx(), colors, ranks,
                          atomOrders, atomRingClosures, bondsInPlay,
                   ...
                   const std::vector<std::string>* bondSymbols, bool doRandom)
{
    ...
    if (atomRingClosures[atomIdx].size()) {
        ...
        for (auto bIdx : atomRingClosures[atomIdx]) {
            ...
            if (bond->getPropIfPresent(
                    common_properties::_TraversalRingClosureBond, ringIdx)) {
                ...
                // don't make the ring digit immediately available again: we
                // don't want to have the same ring digit opening and closing
                // rings on an atom.
                ringsClosed.push_back(ringIdx - 1);
            } else {
                // this is the beginning of the ring closure, we need to come up
                // with a ring index:
                auto cAIt = std::find(cyclesAvailable.begin(),
                                      cyclesAvailable.end(), 1);
                if (cAIt == cyclesAvailable.end()) {
                    throw ValueErrorException("Too many rings open at once. "
                                              "SMILES cannot be generated.");
                }
    ...
    for (auto possiblesIt = possibles.begin(); possiblesIt != possibles.end();
        ...
        if (colors[possibleIdx] != WHITE_NODE) {
            // we're either done or it's a ring-closure, which we already
            // processed... this test isn't strictly required, because we only
            // added WHITE notes to the possibles list, but it seems logical to
            // document it
            continue;
        }
        ...
        if (possiblesIt + 1 != possibles.end()) {
            ...
            molStack.push_back(MolStackElem(
                "(", rdcast<int>(possiblesIt - possibles.begin())));
        }
        molStack.push_back(MolStackElem(bond, atomIdx));
                           ...
                           bool doRandom)
{
    ...
    PRECONDITION(bondVisitOrders.size() >= mol.getNumBonds(),
                 "vector too small");
    ...
    dfsBuildStack(mol, atomIdx, inBondIdx, colors, cycles, ranks,
                  cyclesAvailable, molStack, atomOrders, bondVisitOrders,
                  atomRingClosures, atomTraversalBondOrder, bondsInPlay,
                  bondSymbols, doRandom);
}
...
void clearBondDirs(ROMol& mol, Bond* refBond, const Atom* fromAtom,
                   ...
                   const UINT_VECT&)
{
    PRECONDITION(bondDirCounts.size() >= mol.getNumBonds(),
                 "bad dirCount size");
            ...
            if (canHaveDirection(tBond) &&
                ...
                while (beg != end) {
                    if (mol[*beg] != tBond &&
                        ...
                        mol[*beg]->getStereo() > Bond::STEREOANY) {
                        dblBondAtom =
                            canonEndAtom; // tBond->getOtherAtom(canonEndAtom);
                        break;
                    }
                    beg++;
                }
            ...
            if (atom->getChiralTag() != Atom::CHI_UNSPECIFIED) {
                ...
                for (const auto bnd : mol.atomBonds(atom)) {
                    if (bondsInPlay && !(*bondsInPlay)[bnd->getIdx()]) {
                        atom->setProp(common_properties::_brokenChirality,
                                      true);
                        break;
                    }
                }
                ...
                // Check if the atom can be chiral, and if chirality needs
                // inversion
                const INT_LIST& trueOrder =
                    atomTraversalBondOrder[atom->getIdx()];
                if (trueOrder.size() >= 3) {
                    int nSwaps = 0;
                    int perm = 0;
                    if (Chirality::hasNonTetrahedralStereo(atom)) {
                        atom->getPropIfPresent(
                    ...
                    if (trueOrder.size() < atom->getDegree()) {
                        ...
                        for (const auto bnd : mol.atomBonds(atom)) {
                            ...
                            if (std::find(trueOrder.begin(), trueOrder.end(),
                                          bndIdx) == trueOrder.end()) {
                        ...
                        if (!perm) {
                            ...
                            perm =
                                Chirality::getChiralPermutation(atom, tOrder);
                        }
                    } else {
                        if (!perm) {
                            nSwaps = atom->getPerturbationOrder(trueOrder);
                        } else {
                            perm = Chirality::getChiralPermutation(atom,
                                                                   trueOrder);
                        }
                    }
                    ...
                    if (doChiralInversions &&
                        chiralAtomNeedsTagInversion(
                            mol, atom,
                            molStack.begin()->obj.atom->getIdx() ==
                                atom->getIdx(),
                            atomRingClosures[atom->getIdx()].size())) {
                        // This is a special case. Here's an example:
                        //   Our internal representation of a chiral center is
                        //   equivalent to:
                        //     [C@](F)(O)(C)[H]
            ...
            if (msI.type == MOL_STACK_ATOM &&
                ...
                if (msI.obj.atom->hasProp(
                    ...
                    const INT_VECT& ringStereoAtoms =
                        msI.obj.atom->getProp<INT_VECT>(
                            common_properties::_ringStereoAtoms);
                    for (auto nbrV : ringStereoAtoms) {
                        int nbrIdx = abs(nbrV) - 1;
                        // Adjust the chirality flag of the ring stereo atoms
                        ...
                        if (!ringStereoChemAdjusted[nbrIdx] &&
                            ...
                            // Odd number of swaps for first chiral ring atom
                            // --> needs to be swapped but we want to retain
                            // chirality
                            if (numSwapsChiralAtoms[msI.obj.atom->getIdx()]) {
                                // Odd number of swaps for chiral ring neighbor
                                // --> needs to be swapped but we want to retain
                                // chirality
                            ...
                            // Even number of swaps for first chiral ring atom
                ...
                } else if (size_t sgidx; msI.obj.atom->getPropIfPresent(
                                             "_stereoGroup", sgidx) &&
                                         mol.getStereoGroups().size() > sgidx) {
                    // make sure that the reference atom in the stereogroup is
                    // CCW
                    auto& sg = mol.getStereoGroups()[sgidx];
                    bool swapIt = msI.obj.atom->getChiralTag() ==
    ...
    if ((*refAtom)->getPerturbationOrder(nbrs) % 1) {
      refState = Atom::ChiralType::CHI_TETRAHEDRAL_CW;
        ...
        if (refAtom->getChiralTag() != refState) {
            // we need to flip everyone... so loop over the other atoms and flip
            // them all:
            std::for_each(sgAtoms.begin(), sgAtoms.end(),
                          [](auto atom) { atom->invertChirality(); });
        }

        // note that we do not forward the Group Ids: this is intentional, so
        // that the Ids are reassigned based on the canonicalized order.
