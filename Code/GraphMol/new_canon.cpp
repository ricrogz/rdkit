...
int bondholder::compareStereo(const bondholder& o) const
{
    ...
    if ((st1 == Bond::BondStereo::STEREOE ||
         st1 == Bond::BondStereo::STEREOZ) &&
        (st2 == Bond::BondStereo::STEREOE ||
         st2 == Bond::BondStereo::STEREOZ)) {
        if (st1 < st2) {
            return -1;
        } else if (st1 > st2) {
            return 1;
...
// RefinePartitions(mol,atoms,ftor,false,order,count,activeset,next,changed,touched);
    ...
    for (unsigned i = 0; i < nAts; ++i) {
        if (ringInfo->isInitialized() && ringInfo->numAtomRings(order[i])) {
            ...
            ringAtoms++;
            if (ringInfo->isInitialized() &&
                ringInfo->numAtomRings(order[i]) > 1 && count[order[i]] > 1) {
    ...
    if (useSpecial && ties && ringAtoms > 0 &&
        static_cast<float>(symRingAtoms) / ringAtoms > 0.5 &&
        branchingRingAtom) {
    ...
    if (breakTies) {
        BreakTies(mol, atoms, ftor, true, order, count, activeset, next,
                  changed, touched);
#ifdef VERBOSE_CANON
        std::cerr << "3--------" << std::endl;
        for (unsigned int i = 0; i < mol.getNumAtoms(); ++i) {
            ...
            if (bond->getBeginAtom()->getDegree() > 2) {
                for (const auto nbr :
                    ...
                    if (nbr->getIdx() != bond->getEndAtomIdx() &&
                        nbr->getIdx() != static_cast<unsigned int>(
                                             bond->getStereoAtoms()[0])) {
                    ...
                    std::vector<bondholder>& nbrs)
{
    ...
    while (beg != end) {
        ...
        // STEREOTRANS here isn't actually canonical. In order for that to work
        // we would need to be able to canonicalize the CIS/TRANS assignment,
        ...
        if (bond->getBondType() == Bond::DOUBLE && nbr->getAtomicNum() == 15 &&
            ...
            // "(b) Contributions by d orbitals to bonds of quadriligant atoms
            // are neglected." FIX: this applies to more than just P
        } else {
            nReps = static_cast<unsigned int>(
                floor(2. * bond->getBondTypeAsDouble()));
        }
    ...
    if (!at->needsUpdatePropertyCache()) {
        for (unsigned int ii = 0; ii < at->getTotalNumHs(); ++ii) {
            nbrs.emplace_back(Bond::SINGLE, Bond::STEREONONE,
                              ATNUM_CLASS_OFFSET, ATNUM_CLASS_OFFSET, 0);
            nbrs.emplace_back(Bond::SINGLE, Bond::STEREONONE,
                              ATNUM_CLASS_OFFSET, ATNUM_CLASS_OFFSET, 0);
        }
    }
                            ...
                            bool needsInit)
{
    ...
    if (needsInit) {
        for (const auto bond : mol.bonds()) {
            ...
            begAt.bonds.push_back(makeBondHolder(bond, bond->getEndAtomIdx(),
                                                 includeChirality, atoms));
    ...
    for (size_t i = 0; i < mol.getNumAtoms(); ++i) {
        ...
        if (needsInit) {
            ...
            atomsi.totalNumHs +=
                (mol.getAtomWithIdx(i)->getDegree() - atomsi.degree);
        }

        // and sort our list of neighboring bonds
        std::sort(atomsi.bonds.begin(), atomsi.bonds.end(),
                  bondholder::greater);
    }
}
    ...
    std::vector<std::pair<unsigned int, unsigned int>>& result)
{
    ...
    for (auto& nbr : nbrs) {
        ...
        if (isRingAtom && atoms[nbrIdx].atom->getChiralTag() != 0) {
            ...
            if (atoms[nbrIdx].atom->getChiralTag() ==
...
{
    PRECONDITION(atomsInPlay.size() == mol.getNumAtoms(),
                 "bad atomsInPlay size");
    PRECONDITION(bondsInPlay.size() == mol.getNumBonds(),
                 "bad bondsInPlay size");
    PRECONDITION(!atomSymbols || atomSymbols->size() == mol.getNumAtoms(),
