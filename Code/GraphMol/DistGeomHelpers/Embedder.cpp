    ...
    for (unsigned int i = 0; i < atoms.size() - 1; ++i) {
        for (unsigned int j = i + 1; j < atoms.size(); ++j) {
            ...
            if (((d2 < lb) && (fabs(d2 - lb) > 0.1 * ub)) ||
                ...
                std::cerr << a1 << " " << a2 << ":" << d2 << " " << lb << " "
                          << ub << " " << fabs(d2 - lb) << " " << fabs(d2 - ub)
                          << std::endl;
#endif
                return false;
            }
    ...
    if (!embedParams.useRandomCoords) {
        ...
        if (embedParams.useRandomCoords && embedParams.coordMap != nullptr) {
            for (const auto& v : *embedParams.coordMap) {
                ...
                for (unsigned int ci = v.second.dimension();
                     ci < p->dimension(); ++ci) {
                    (*p)[ci] = 0.0;
                }
            }
                             ...
                             const EmbedParameters& embedParams)
{
    ...
    std::unique_ptr<ForceFields::ForceField> field2(
        DistGeom::constructForceField(*eargs.mmat, *positions,
                                      *eargs.chiralCenters, 0.2, 1.0, nullptr,
                            ...
                            const detail::EmbedArgs& eargs, EmbedParameters&)
{
    for (const auto& itm : *eargs.stereoDoubleBonds) {
        ...
        // So multiplying the result of the subtracion from the second element
        // of
        //   the dihedralBond element will give a positive value if the dihedral
        //   is closer to correct than it is to incorrect and a negative value
        //   otherwise.
...
#endif
                embedParams
                    .failures[EmbedFailureCauses::FINAL_CENTER_IN_VOLUME]++;
            }
            return false;
        }
    }
    ...
    while (!gotCoords && iter < embedParams.maxIterations) {
        ...
        if (!gotCoords) {
...
#endif
                    embedParams
                        .failures[EmbedFailureCauses::FIRST_MINIMIZATION]++;
                }
            } else {
                gotCoords = EmbeddingOps::checkTetrahedralCenters(
                    positions, eargs, embedParams);
                if (!gotCoords) {
                    if (embedParams.trackFailures) {
#ifdef RDK_BUILD_THREADSAFE_SSS
                        std::lock_guard<std::mutex> lock(GetFailMutex());
            ...
            if (gotCoords && (embedParams.useExpTorsionAnglePrefs ||
                ...
                gotCoords = EmbeddingOps::minimizeWithExpTorsions(
                    *positions, eargs, embedParams);
                if (!gotCoords) {
                    if (embedParams.trackFailures) {
#ifdef RDK_BUILD_THREADSAFE_SSS
...
#endif
                        embedParams
                            .failures[EmbedFailureCauses::ETK_MINIMIZATION]++;
                    }
                }
            }
            if (gotCoords) {
                gotCoords = EmbeddingOps::doubleBondGeometryChecks(
                    *positions, eargs, embedParams);
...
#endif
                    embedParams
                        .failures[EmbedFailureCauses::LINEAR_DOUBLE_BOND]++;
                }
            }
            // test if stereo is correct
            if (embedParams.enforceChirality && gotCoords) {
                if (!eargs.chiralCenters->empty()) {
                    // test if chirality is correct. Any additional test
                    // failures will be tracked there if necessary.
                    gotCoords = EmbeddingOps::finalChiralChecks(
                        positions, eargs, embedParams);
                }
                if (gotCoords && !eargs.stereoDoubleBonds->empty()) {
                    gotCoords = EmbeddingOps::doubleBondStereoChecks(
                        *positions, eargs, embedParams);
                    if (!gotCoords && embedParams.trackFailures) {
#ifdef RDK_BUILD_THREADSAFE_SSS
                        std::lock_guard<std::mutex> lock(GetFailMutex());
    ...
    for (const auto bnd : mol.bonds()) {
        if (bnd->getBondType() == Bond::BondType::DOUBLE) {
            ...
            if (bnd->getStereo() > Bond::BondStereo::STEREOANY) {
                ...
                if (coordMap &&
                    coordMap->find(bnd->getStereoAtoms()[0]) !=
                        coordMap->end() &&
                    coordMap->find(bnd->getStereoAtoms()[1]) !=
                        coordMap->end()) {
                    continue;
                }
                int sign = 1;
    ...
    for (const auto& atom : mol.atoms()) {
        if (atom->getAtomicNum() != 1) { // skip hydrogens
            ...
            if ((chiralType == Atom::CHI_TETRAHEDRAL_CW ||
                ...
                // now create a chiral set and set the upper and lower bound on
                // the volume
                if (chiralType == Atom::CHI_TETRAHEDRAL_CCW) {
                    // positive chiral volume
                    auto* cset = new DistGeom::ChiralSet(
                        atom->getIdx(), nbrs[0], nbrs[1], nbrs[2], nbrs[3],
                        volLowerBound, volUpperBound);
                    DistGeom::ChiralSetPtr cptr(cset);
                    chiralCenters.push_back(cptr);
                } else if (chiralType == Atom::CHI_TETRAHEDRAL_CW) {
                    auto* cset = new DistGeom::ChiralSet(
                        atom->getIdx(), nbrs[0], nbrs[1], nbrs[2], nbrs[3],
                        -volUpperBound, -volLowerBound);
                    DistGeom::ChiralSetPtr cptr(cset);
                    chiralCenters.push_back(cptr);
                } else {
                    if ((coordMap &&
                         coordMap->find(atom->getIdx()) != coordMap->end()) ||
                        (mol.getRingInfo()->isInitialized() &&
                         (mol.getRingInfo()->numAtomRings(atom->getIdx()) < 2 ||
                          mol.getRingInfo()->isAtomInRingOfSize(atom->getIdx(),
                                                                3)))) {
                        // we only want to these tests for ring atoms that are
                        // not part of the coordMap there's no sense doing
                        // 3-rings because those are a nightmare
                    } else {
                        auto* cset = new DistGeom::ChiralSet(
                            atom->getIdx(), nbrs[0], nbrs[1], nbrs[2], nbrs[3],
                            0.0, 0.0);
...
void adjustBoundsMatFromCoordMap(DistGeom::BoundsMatPtr mmat, unsigned int,
                                 const std::map<int, RDGeom::Point3D>* coordMap)
{
    ...
    ForceFields::CrystalFF::CrystalFFDetails& etkdgDetails)
{
    ...
    unsigned int nAtoms = mol->getNumAtoms();
    if (params.useExpTorsionAnglePrefs || params.useBasicKnowledge) {
        setTopolBounds(*mol, mmat, etkdgDetails.bonds, etkdgDetails.angles,
...
template <class T> bool multiplication_overflows_(T a, T b)
{
        ...
        if (new_seed > -1) {
            if (params->enableSequentialRandomSeeds) {
                ...
                if (!multiplication_overflows_(rdcast<int>(ci + 1),
                    ...
                    // only grab the first 31 bits xor'd with the next 31 bits
                    // to make sure its positive, careful, the 'ULL' is
                    // important here, 0x7fffffff is the 'int' type because of C
                    // default number semantics and that we definitely don't
                    // want!
        ...
        if (gotCoords) {
            ...
            for (unsigned int i = 0; i < conf->getNumAtoms(); ++i) {
                if (!eargs->fragMapping ||
                    (*eargs->fragMapping)[i] ==
                        static_cast<int>(eargs->fragIdx)) {
                    conf->setAtomPos(
                        i, RDGeom::Point3D((*positions[fragAtomIdx])[0],
...
getMolSelfMatches(const ROMol& mol, const EmbedParameters& params)
{
                        ...
                        EmbedParameters& params)
{
    ...
    for (unsigned int fragIdx = 0; fragIdx < molFrags.size(); ++fragIdx) {
        ...
        if (params.boundsMat == nullptr || molFrags.size() > 1) {
            // The user didn't provide one, so create and initialize the
            // distance bounds matrix
            mmat.reset(new DistGeom::BoundsMatrix(nAtoms));
            initBoundsMat(mmat);
            if (!EmbeddingOps::setupInitialBoundsMatrix(
                    piece.get(), mmat, coordMap, params, etkdgDetails)) {
            ...
            if (params.boundsMat->numRows() != nAtoms) {
                throw ValueErrorException("size of boundsMat provided does not "
                                          "match the number of atoms in "
                                          "the molecule.");
            }
        ...
        if (numThreads == 1) {
            ...
            for (int tid = 0; tid < numThreads; ++tid) {
                tg.emplace_back(std::async(std::launch::async,
                                           detail::embedHelper_, tid,
                                           numThreads, &eargs, &params));
            }
            for (auto& fut : tg) {
                fut.get();
            }
