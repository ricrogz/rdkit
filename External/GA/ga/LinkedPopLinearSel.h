...
class LinkedPopLinearSel
{
  private:
    ...
    bool
    addToPopulation(std::multimap<double, std::shared_ptr<Chromosome>>& pop,
  ...
  public:
    ...
    getTiedBest(double tolerance = 1e-6) const;
...
LinkedPopLinearSel<Chromosome, PopulationPolicy>::LinkedPopLinearSel(
    ...
    : populationPolicy(populationPolicy_), rng(populationPolicy.getRng()),
      popsize(populationPolicy.getPopsize()),
      operations(populationPolicy.getOperations()),
      selectionPressure(populationPolicy.getSelectionPressure()), population(),
      freeChromosomes()
{
    ...
    // selection pressure is defined as the ratio between the fitness of the
    // worst individual to the best individual.
    ...
    (void) predictTotalScaledFitness;
#ifdef INCLUDE_REPORTER
    REPORT(Reporter::TRACE)
        << "totalScaledFitness " << totalScaledFitness
        << " predictTotalScaledFitness " << predictTotalScaledFitness;
#endif
    assert(std::abs(totalScaledFitness - predictTotalScaledFitness) < 1.0e-5);

    double predictEndFitness =
...
#ifdef INCLUDE_REPORTER
    REPORT(Reporter::TRACE)
        << "Inserted a new chromosome, fitness " << fitness << " popsize "
        << pop.size() << " " << chromosome->info();
#endif
...
std::string LinkedPopLinearSel<Chromosome, PopulationPolicy>::info() const
{
    boost::format format =
        boost::format(
            "Op %5d Fit %7.3f Added %5d Dups %5d Niche %5d Fail %4d") %
...
template <typename Chromosome, typename PopulationPolicy>
std::string
LinkedPopLinearSel<Chromosome, PopulationPolicy>::populationInfo() const
{
    std::string rtn;
    int no = 1;
